...
async def get_businesses():
    ...
    # remove dunder dentoted fields from response
    # it seems like ObjectID is not needed and causes
    # issue for the serialization into json

    filtered_businesses = []

    # TODO: THIS IS FUCKING HORRID PLZ FIX
...
async def send_signup_mail(request: Request):
    ...
    if not DEBUG:
        response = requests.post(
            url=recaptcha_url,
            data={"secret": recaptcha_secret_key, "response": json["captchaValue"]},
        ).json()
    ...
    slug = (
        "new_pwd_redirect?access_token=" + sign_up_token
    )  # new_pwd_token + "&email=" + email.replace("@", "%40")
    signup_url = base_url + slug

    subject = "Signup to coworkingfriendly.com"
    body = (
        "To complete registration and set your account password use this link:\n"
        + signup_url
        + "\n Cheers and have fun!"
    )
...
@app.get(
    "/api/user/details",
    dependencies=[
        Depends(JWTBearer([{"role": "sign_up"}, {"role": "user"}, {"role": "admin"}]))
    ],
    tags=["user_management"],
)
...
def set_http_only_cookie(response, key, value):
    ...
    if not DEBUG:
        response.set_cookie(
            key=key, value=value, httponly=True, secure=True, samesite="strict"
        )
    else:
